 ...
 * \author Robert Marshall KI4MCW <ki4mcw "at" gmail "dot" com>
 *
 */
...
static void ax25_decode_kiss(AX25Ctx *ctx)
{
        // No need to decode data, since it is already very close
        // to the format req'd for KISS. Just truncate the checksum,
        // pass the data over to the callback function, and we're done.

	if (ctx->hook)
		ctx->hook( ctx->frm_len - 2 );
        else
                LOG_ERR( "No callback specified for KISS output!" ) ;
}


static void ax25_decode_parsed(AX25Ctx *ctx)
{
	AX25Msg msg;
	uint8_t *buf = ctx->buf;
 ...
 * \param fmt Format in which to pass data to callback function.
 */

void ax25_poll(AX25Ctx *ctx, const enum AX25OutputFormat fmt)
{
	int c;
	...
	while ((c = kfile_getc(ctx->ch)) != EOF)
	{
			...
			if (ctx->frm_len >= AX25_MIN_FRAME_LEN)
			{
				if (ctx->crc_in == AX25_CRC_CORRECT)
				{
					LOG_INFO("Frame found!\n");
                                        // 20101128 Robert Marshall KI4MCW
                                        // Choice of output formats
                                        if (fmt == AX25_OUTPUT_PARSED)
					    ax25_decode_parsed(ctx);
                                        else if (fmt == AX25_OUTPUT_KISS)
                                            ax25_decode_kiss(ctx);
				}
				else
				{
        ...
        if ( ctx->sync )
        {
            for ( xx=14 ; xx < ctx->frm_len ; xx+=7 )
            { 
                if ( ctx->buf[ xx ] == 0x03 ) 
                { gotdcd = true ; break ; }
            }    
        }
        if ( gotdcd ) { PORTB |=  0x01 ; }
        else          { PORTB &= ~0x01 ; }
...
static void ax25_sendCall(AX25Ctx *ctx, const AX25Call *addr, bool last)
{
	...
	/* The bit0 of last call SSID should be set to 1 */
        ...
        // 20101124 RM - SSID is sent as a char, not as an int
        uint8_t ssid = addr->ssid << 1 | (last ? 0x01 : 0) | 0x60 ;
