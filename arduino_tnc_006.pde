...
void setup()
{
    ...
    // use 16-bit Timer3 (not Timer1 - conflicts with Arduino delay fns)
    // to check on things at 11x the 1200bps signalling rate (13200 Hz)
    TCCR3A = 0x00 ;
    ...
    // use blinky on ArduinoMEGA as DCD light (pin 13)
    DDRB = 0xC0 ;
    ...
    Serial.print("Robert Marshall KI4MCW - Arduino TNC v") ;
...
ISR(TIMER3_COMPA_vect)
{
    if ( hb12==LOW ) { hb12 = HIGH ; PORTB |= 0x40 ; }
    else             { hb12 = LOW  ; PORTB &= ~0x40 ; }
    ...
    case 7: if ( bitqlen > 1 )                // binary "0111111"
            {
	    ...
	    thesebits = 0 ;
            // drop any accumulated bit counts
            bittimes = 0 ;
	    break ;
